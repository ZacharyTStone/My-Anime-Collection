 ...
 */
export function mapFetchedAnime(anime: ExpectedFetchedAnimeResponse): SavedAnime {
  const attrs = anime.attributes;
  return {
    _id: anime.id || "",
    title: attrs?.titles?.en || attrs?.titles?.en_jp || attrs?.canonicalTitle || "Title N/A",
    ...
    synopsis: attrs?.synopsis ? DOMPurify.sanitize(attrs.synopsis) : "No synopsis available",
    coverImage: attrs?.posterImage?.medium || attrs?.posterImage?.small || "",
    youtubeVideoId: attrs?.youtubeVideoId ?? undefined,
    japanese_title:
      attrs?.titles?.ja_jp || attrs?.titles?.en_jp || attrs?.canonicalTitle || "Title N/A",
    fetchedAnime: anime,
  };
}
